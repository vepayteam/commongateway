...
$schet = "
	...
	<table cellpadding='0' cellspacing='0'  width='100%' class='table-acc'>
		<tr>
			...
			<td style='vertical-align: bottom;' rowspan='2'>".$RaschSchet."</td>
		</tr>
		<tr>
			<td colspan='2'>Получатель<br>".$UrLicoPostP."</td>
		</tr>
		<tr>
			<td colspan='2' rowspan='2'>Банк получателя<br>".$NameBank."</td>
			<td >БИК</td>
			<td>".$BicBank."</td>
		</tr>
		<tr>
	...
	<div style='font-weight:bold;padding: 24px 0 6px; font-size: 16px;'>Счет № ".$Schet->NumSchet." от ".date('d.m.Y',$Schet->DateSchet)."</div>

	<table style='width:100%;padding-top:8px; margin-left:-3px;'>
		<tr>
			<td style='font-size: 14px;padding-bottom:5px;'>Поставщик:</td>
			<td style='font-size: 14px;padding-bottom:5px;'>".$UrLicoPostP." ИНН ".$InnP.", КПП ".$KppP.", ".$AddrP."</td>
		</tr>
		<tr>
			<td style='font-size: 14px;'>Покупатель:</td>
			<td style='font-size: 14px;'>".$Partner->UrLico." ИНН ".$Partner->INN." КПП ".$Partner->KPP."</td>
		</tr>
	</table>
	...
	<div style='padding-bottom: 20px;font-size: 14px'>Оплата по счету ".$Schet->NumSchet." от ".date('d.m.Y',$Schet->DateSchet).", ".Html::decode($Schet->Komment).". Без НДС.</div>
	<table cellpadding='1' cellspacing='0' width='100%' class='table-acc' >
		...
		<tr>
			...
			<td style='vertical-align: top; text-align: left;'>".Html::decode($Schet->Komment)."</td>
	...
	<div style='width:100%;'>
		<div style='float:left;padding: 15.25pt 0 0 0;font-size: 14px'>Руководитель предприятия____________ (".$DirP.")</div>
		<div style='float:right;padding: 15.25pt 0 0 0;font-size: 14px'>Главный бухгалтер____________ (".$BuhP.")</div>
	</div>";
?>
