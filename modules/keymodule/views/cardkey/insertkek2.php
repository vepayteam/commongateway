    ...
    <div class="col-lg-12">
        <div class="ibox">
            ...
            <div class="ibox-content">
                <form id="insertkey2" class="form-horizontal">
                    <div class="row m-b-xl m-t-sm">
                        <div class="col-sm-12">
                            <div><label>Ключ 2</label></div>
                            <div><input type="text" class="form-control" name="InsertKey[key2]" value="" autocomplete="off"></div>
                        </div>
                    </div>
                    <div class="row m-b-xl m-t-sm">
                        <div class="col-sm-12">
...
<script>
    let insertKey = function () {
        $('#insertkey2').on('submit', function () {

            if (!confirm("Подтвердите занесение ключ?")) return false;

            $.ajax({
                type: "POST",
                url: '/keymodule/cardkey/savekek2',
                data: $('#insertkey2').serialize(),
                beforeSend: function () {
                },
                success: function (data) {
                    if (data.status == 1) {
                        toastr.success("OK", "Ключ 2 внесён");
                        $('#insertkey2').trigger("reset");
                    } else {
