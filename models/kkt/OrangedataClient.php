...
class OrangedataClient
{
    ...
    public function add_position_to_order(array $params = [])
    {
        ...
        if ($supplierInfo) {
            ...
            if ($supplierInfo['phoneNumbers']) {
                for ($i = 0, $iMax = count($supplierInfo['phoneNumbers']); $i < $iMax; $i++) {
        ...
        if ($agentInfo) {
            if ($agentInfo['paymentTransferOperatorPhoneNumbers']) {
                for ($i = 0, $iMax = count($agentInfo['paymentTransferOperatorPhoneNumbers']); $i < $iMax; $i++) {
            ...
            if ($agentInfo['paymentAgentPhoneNumbers']) {
                for ($i = 0, $iMax = count($agentInfo['paymentAgentPhoneNumbers']); $i < $iMax; $i++) {
            ...
            if ($agentInfo['paymentOperatorPhoneNumbers']) {
                for ($i = 0, $iMax = count($agentInfo['paymentOperatorPhoneNumbers']); $i < $iMax; $i++) {
    ...
    public function add_agent_to_order(array $params = [])
    {
        ...
        for ($i = 0, $iMax = count($paymentTransferOperatorPhoneNumbers); $i < $iMax; $i++) {
        ...
        for ($i = 0, $iMax = count($supplierPhoneNumbers); $i < $iMax; $i++) {
    ...
    public function create_correction(array $params = [])
    {
        ...
        $this->correction_request->content->causeDocumentDate = $causeDocumentDate->setTime(0, 0)->format(DateTime::ATOM);
