...
class KfBenific extends Model
{
    ...
    {
        $this->uraddres = (object)$this->uraddres;
        $this->actualaddres = (object)$this->actualaddres;
     ...
     * @return string
     */
    public function GetSoapForm($partner)
    {
        ...
        if ($this->registrationtype == self::SOLE_PROPRIETOR_TYPE_REG) {
            ...
            if ($this->uraddres) {
                ...
                $RegistrationAddress->appendChild($dom->createElement("FullAddress", $this->GetFullAdress($this->uraddres)));
        ...
        if ($this->registrationtype == self::LEGAL_PERSON_TYPE_REG) {
            ...
            if ($this->uraddres) {
                ...
                $JuridicalAddress->appendChild($dom->createElement("FullAddress", $this->GetFullAdress($this->uraddres)));
            ...
            if ($this->actualaddres) {
                ...
                $ActualAddress->appendChild($dom->createElement("FullAddress", $this->GetFullAdress($this->actualaddres)));
        ...
        if ($this->registrationtype == self::INDIVIDUAL_TYPE_REG) {
            ...
            if ($this->actualaddres) {
                ...
                $FactAddress->appendChild($dom->createElement("FullAddress", $this->GetFullAdress($this->actualaddres)));
        ...
        $beneficiary->appendChild($dom->createElement("accountNumber", $partner->SchetTcbNominal));
    ...
    private function GetFullAdress($addres)
    {
        $adr = $addres->region . ' ' . $addres->regiontype;
        if (!empty($addres->city)) {
            $adr .= ', ' . $addres->citytype . ' ' . $addres->city;
        }
        if (!empty($addres->place)) {
            $adr .= ', ' . $addres->placetype . ' ' . $addres->place;
        }
        if (!empty($addres->street)) {
            $adr .= ', ' . $addres->streettype . ' ' . $addres->street;
        }
        if (!empty($addres->house)) {
            $adr .= ', д ' . $addres->house;
        }
        if (!empty($addres->corp)) {
            $adr .= ' ' . $addres->corp;
        }
        if (!empty($addres->flat)) {
            $adr .= ', ' . $addres->flat;
        }
